...
- Un cadre d'étude intéressant est aussi l'#link("https://fr.wikipedia.org/wiki/Arithm%C3%A9tique_de_Presburger")[Arithmétique Linéaire Entière, dite de "Presburger"], beaucoup plus proche de l'Arithmétique Linéaire Rationnelle, à l'exception de la division par $c$ qui n'est plus permise, cela créerait de nouveaux problèmes à résoudre dans notre procédure.
...
Cela ressemble à de l'algèbre linéaire : en supposant qu'il n'y a que des égalités dans une clause, on tombe sur un sous-espace affine (ici, de dimension 1), 
alors rajouter des inégalités va créer une forme géométrique. On peut parler de #link("https://lalgebrisant.fr/images/pdfArticles/LePolyedreVide.pdf")["polyèdre convexe"]. 

Trouver un point extérieur est intuitif : le polyèdre étant défini par des "facettes", il suffirait de choisir une valeur de $y$ qui permet de franchir l'une de ces bornes. 

L'ordinateur peut en réalité prendre directement l'union des polyèdres/sous-espaces affines générés par $psi_1, psi_2, psi_3$ afin de les invalider par la théorie des ordres denses sans bornes; l'élimination des quantificateurs va projeter l'union de formes géométriques et la projeter sur l'axe de $y$. 
Ainsi, $O$ gagne forcément, car la subsitution d'une variable quantifiée par une constante n'est qu'une projection de cet ensemble de formes sur une dimension plus faible. Il peut ainsi jouer une valeur quelconque jusqu'à la fin. 

Cette approche géométrique (intersections de polyèdres/sous-espaces affines et projections) correspond exactement à ce que calcule l'algorithme d'élimination des quantificateurs avec Fourier-Motzkin et Gauss-Jordan implémentés dans le Lot 2.
